...
function buttonPress() {
  ...
  if (event.key === ' ' || event.target.id == "dialogue-arrow") {
    ...
    //check if you've found an interactable object

    //if so, we need to open the dialogue popup

    //do the item specific code if it's an item

    //get the dialogue from the list
    //use the first 39 words

    //Check if you're talking to an NPC as well
    var interactFound = interactCheck(words);

    if (interactFound != undefined) {
